...
def process_query(query, dictionary, docID_index):
    """
...
if __name__ == "__main__":
    ...
    print(" - search *BOOLEAN INFORMATION* : searche the boolean expression. Example : (A OR B) AND C")

    while shell_open:
        str_request = input(">> ")
        if str_request == "exit()":
            shell_open = False
        elif str_request.split()[0] == "search":
            query = " ".join(str_request.split()[1:])
            print("raw query : ", query)
            print(process_query(query, dictionary, docID_index))
